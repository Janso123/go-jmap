...
func TestQueryChangesInvoke(t *testing.T) {
	...
	assert.Equal(t,
		`{"using":["urn:ietf:params:jmap:principals"],"methodCalls":[["Principal/queryChanges",{"accountId":"u1","sinceQueryState":"s1","maxChanges":50,"filter":{"email":"jane@example.com"},"sort":[{"property":"email"}]},"0"]]}`,
		string(data))
}
